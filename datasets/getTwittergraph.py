...
def main(obj):
    treePath = os.path.join(cwd, 'data/TWITTER/' + obj + '/bigcn_raw/data.TD_RvNN.vol_5000.txt')
    ...
    labelPath = os.path.join(cwd, "data/TWITTER/" + obj + "/bigcn_raw/" + obj + "_label_All.txt")
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez( os.path.join(cwd, 'data/TWITTER/'+obj+'/bigcn_processed/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return 1
